...
# 1. Actualizar llaves de Arch, actualizar sistema e instalar dependencias
...
# Enlace simbólico para mantener compatibilidad de rutas con S6 v2
RUN ln -s /command/with-contenv /usr/bin/with-contenv

# 3. Instalar Bashio con intérprete completo
...
# 4. Crear usuario 'builder' y ajustar makepkg (BUILDDIR en disco y desactivar !strip !debug)
...
# 5. Copiar scripts desde la estructura del proyecto, sanitizar saltos de línea (CRLF a LF) y dar permisos
COPY Hassio-pacman-cherry/Scripts/env.sh /env.sh
COPY Hassio-pacman-cherry/Scripts/run.sh /run.sh
COPY Hassio-pacman-cherry/Scripts/builder.sh /builder.sh
